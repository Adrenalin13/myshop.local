...
<table border="1" cellpadding="1" cellspacing="1">
    ...
    {foreach $rsCategories as $item name=categories}
        <tr>
            <td>{$smarty.foreach.categories.iteration}</td> {*это вызавается порядковый номер*}
            <td>{$item['id']}</td>
            ...
            <td>
                <select id="parentId_{$item['id']}">
                    ...
                    <option value="{$mainItem['id']}" {if $item['parent_id'] ==
                    $mainItem['id']}selected{/if}>{$mainItem['name']}
                        {/foreach}
                </select>
            </td>
    ...
    {/foreach}
</table>   {* далее в CategoriesModel создаем ф-цию по изменению данных категорий*}