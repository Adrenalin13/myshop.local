...
</table>    {* далее создадим ф-цию в ProductModels, сохраняющую введенные тут данные в БД*}

{*Шаг 7 добавляем таблицу редактирования товаров*}
<table border="1" cellpadding="1" cellspacing="1">
        ...
        <tr>
            ...
            <td>
                <select id="itemCatId_{$item['id']}">
                    <option value="0">Главная категория
                        {foreach $rsCategories as $itemCat}
                    <option value="{$itemCat['id']}" {if $item['category_id'] ==
                    $itemCat['id']}selected{/if}>{$itemCat['name']}
                        {/foreach}
            </td>
    ...
    {/foreach}
</table> {*далее в ProductsModel создадим ф-цию обновления редактируемых продуктов*}